...
def a_weighted(audio, sample_rate, hop_length=None, pad=True):
    ...
    if sample_rate != paddlecrepe.SAMPLE_RATE:
        audio = resampy.resample(audio, sample_rate, paddlecrepe.SAMPLE_RATE)
        hop_length = int(hop_length * paddlecrepe.SAMPLE_RATE / sample_rate)
    ...
    stft = librosa.stft(audio,
                        n_fft=paddlecrepe.WINDOW_SIZE,
                        hop_length=hop_length,
                        win_length=paddlecrepe.WINDOW_SIZE,
                        center=pad,
                        pad_mode='constant')
...
def perceptual_weights():
    ...
    frequencies = librosa.fft_frequencies(sr=paddlecrepe.SAMPLE_RATE,
                                          n_fft=paddlecrepe.WINDOW_SIZE)
